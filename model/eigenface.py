...
def compare_faces(image1_path, image2_path, threshold=0.2800):
    ...
    # Menghitung Jarak Euclidean Jarak Vektor Bobot Proyeksi
    euclidean_dist = float(np.linalg.norm(weight1 - weight2))

    similarity_percentage = round((max(-1, min(1, cosine_sim)) + 1) / 2 * 100, 2)

    if cosine_sim >= threshold:
        result = "Kemungkinan Orang Yang Sama (Identitas COCOK)"
    else:
        result = "Kemungkinan Orang Berbeda (Identitas TIDAK COCOK)"

    return (similarity_percentage, round(cosine_sim, 4), round(euclidean_dist, 4), threshold, result)